...
Pilha *inverte(Pilha *pilhaOrig) {
  Pilha *pilhaInv = inicializaPilha();

  int tam = pilhaOrig->tamanho;
  for (int i = 0; i < tam; i++) {
    Site *site = pop(pilhaOrig);
    push(pilhaInv, site);
  }
